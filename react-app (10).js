...
const customStyles = {
  ...
  stripedAwning: {
    background: 'repeating-linear-gradient(90deg, #22C55E, #22C55E 15px, #16A34A 15px, #16A34A 30px)',
  },
  upgradeButton: {
...
const App = () => {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      #app-container-inner::-webkit-scrollbar {
        display: none;
      }
      @keyframes pulse {
        0%, 100% { opacity: 1; }
  ...
  const handleMapClick = () => {
    setMapClicked(true);
    setTimeout(() => setMapClicked(false), 200);
  };
  ...
  return (
    <div style={customStyles.body}>
      ...
      <link href="https://unpkg.com/@phosphor-icons/web@2.1.1/src/index.css" rel="stylesheet" />
      ...
      >
        ...
        <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md px-6 py-4 flex items-center justify-between border-b border-slate-100">
          ...
          <button className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600">
            <i className="ph-bold ph-dots-three-outline-fill text-xl"></i>
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="relative w-full h-56 bg-gradient-to-b from-blue-50 to-white rounded-3xl flex flex-col items-center justify-center overflow-hidden border border-slate-100">
            ...
            <div className="relative z-10 flex flex-col items-center animate-float">
              <div className="absolute -top-6 z-30 w-12 h-12 bg-white rounded-full border-[3px] border-green-500 shadow-lg flex items-center justify-center transform rotate-[-5deg]">
                <i className="ph-fill ph-coffee text-green-500 text-xl"></i>
              </div>
              <div className="w-36 h-28 bg-white rounded-xl relative shadow-xl border-2 border-white flex flex-col items-center">
                <div
                  className="w-[110%] h-8 rounded-t-xl rounded-b-md shadow-md z-20 absolute -top-1 border-b-2 border-green-700"
                  style={{ ...customStyles.stripedAwning, left: '-5%' }}
                ></div>
                <div className="w-full px-4 pt-10 flex justify-between relative z-10">
                  ...
                  <div className="w-10 h-16 bg-slate-400 rounded-t-lg border border-slate-300 border-b-0 relative self-end" style={{ marginBottom: '-2px' }}>
                    <div className="absolute top-1/2 right-1.5 w-1.5 h-1.5 bg-white rounded-full"></div>
                  </div>
                </div>
              </div>
              <div className="w-40 h-3 bg-black/5 rounded-[100%] mt-2" style={{ filter: 'blur(4px)' }}></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="flex items-center gap-1 text-slate-700 font-bold">
                <i className="ph-fill ph-map-pin text-yellow-400"></i>
                <span>5th Ave</span>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-end justify-between h-32 gap-2">
              {[
                ...
                <div key={index} className="flex-1 flex flex-col items-center gap-2">
                  <div
                    className={`w-full ${bar.opacity} rounded-t-lg`}
          ...
          <div className="space-y-4">
            ...
            <div className="w-full bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-1 relative border-[1.5px] border-green-500 border-dashed">
              <div
                className="absolute w-4 h-4 bg-[#F8FAFC] rounded-full transform -translate-y-1/2 border-r-[1.5px] border-green-500 border-dashed"
                style={{ top: '50%', left: '-8px' }}
              ></div>
              <div
                className="absolute w-4 h-4 bg-[#F8FAFC] rounded-full transform -translate-y-1/2 border-l-[1.5px] border-green-500 border-dashed"
              ...
              <div className="bg-white/60 rounded-xl px-5 py-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="bg-green-500 text-white font-black px-4 py-2 rounded-xl text-sm flex items-center gap-1 transition-all active:translate-y-1"
                  style={customStyles.upgradeButton}
                >
            ...
            <div className="w-full bg-gradient-to-r from-blue-50 to-sky-50 rounded-2xl p-1 relative border-[1.5px] border-sky-400 border-dashed">
              <div
                className="absolute w-4 h-4 bg-[#F8FAFC] rounded-full transform -translate-y-1/2 border-r-[1.5px] border-sky-400 border-dashed"
                style={{ top: '50%', left: '-8px' }}
              ></div>
              <div
                className="absolute w-4 h-4 bg-[#F8FAFC] rounded-full transform -translate-y-1/2 border-l-[1.5px] border-sky-400 border-dashed"
                style={{ top: '50%', right: '-8px' }}
              ></div>
              <button
                onClick={handleCollect}
                className="w-full bg-white/60 rounded-xl px-5 py-4 flex items-center justify-between"
              >
                <div className="text-left">
                  <p className="font-black text-slate-800 text-base">COLLECT EARNINGS</p>
                  <p className="text-xs font-bold text-slate-500">
                    {collected ? 'Collected!' : 'Ready to claim'}
                  </p>
                </div>
          ...
          <button
            ...
            style={
              mapClicked
                ? { boxShadow: 'none', transform: 'translateY(6px)' }
                : customStyles.viewMapButton
            }
          >
          ...
          <p className="text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest pb-8">
            Owner ID: 0x82f...a9c4
          </p>
        </div>
      </div>
