                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (data.owned) {
                          toast.error(`Already owned by ${data.owner}`);
                          setCheckingOwnership(false);
                          return;
                        }
                      } catch { /* fail open */ }
                      setCheckingOwnership(false);
                      navigate("/camera", { state: { business: selectedBusiness } });
                    }}
                    className="w-full bg-green-500 text-white py-4 rounded-2xl font-black text-lg uppercase tracking-wider hover:bg-green-600 transition-all active:translate-y-1 disabled:opacity-50"
                    style={{ boxShadow: "0 6px 0 #15803D" }}
                  >
                    {checkingOwnership ? "CHECKING..." : "CAPTURE NOW"}
                  </button>
                </>
              )}
            </motion.div>
